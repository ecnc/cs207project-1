...
class BinaryNodeRef(ValueRef):
    """"
    A reference to a btree node on the disk. Secialized subclass of ValueRef to 
...
class BinaryTree(object):
    """"
    Immutable Binary Tree class. Constructs a new tree upon changes
    ...
    def delete(self, key):
        """delete node with given key, creating new tree and path"""
        if self._storage.lock():
            self._refresh_tree_ref()
        node = self._follow(self._tree_ref)
...
class Storage(object):
    ...
    def write(self, data):
        """write data to disk, and return the adress at which it is written"""
